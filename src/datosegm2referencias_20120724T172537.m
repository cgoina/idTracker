...
% 01-Jun-2012 13:01:33 Evito que haya referencias de m�s de
% nframes_final frames. Adem�s cambio la forma de recortar las referencias,
% que era al azar, y paso a que est�n equiespaciados
% 13-Feb-2012 10:25:27 Corrijo que nframes_final s�lo se calculara para las
...
% 06-Oct-2011 16:34:41 Lo preparo para m�s de dos peces
% 03-Aug-2011 21:25:07 En vez de volver atr�s por cada nueva referencia
% incorporada, hace varias pasadas. As� va m�s r�pido.
% 03-Aug-2011 18:55:34 Hago que cuando incorpora una nueva referencia
% vuelva atr�s a intentar recuperar las que se dej� por el camino
% 03-Aug-2011 17:51:38 CORRECCI�N: Hago que ref2_act=referencias2, en vez
% de referencias. Antes el funcionamiento era completamente err�tico.
...
if nargin<3 || isempty(nframes_final)
    nframes_final=500; % N�mero de frames que queremos en la referencia
end
...
%     fprintf('%g,',c_frames)
    ...
    for c_peces=1:n_peces
        ...
        end % if mapa v�lido para referencias
    end % c_peces
end % c_frames
% Recorto
for c_peces=1:n_peces
...
% en la vertical, habr� algunos frames que hayan pasado de uno
% al otro lado
nframes1=min(c_refs);
...
nframes_min=20; % M�nimo de frames que tiene que tener un intervalo para que pueda formar parte de la referencia
algunobueno=true;
while algunobueno
    algunobueno=false;
    c_intervalos=0;
    while c_intervalos<size(intervalosbuenos.iniciofinal,1) && nframes1<nframes_final
        ...
        if ~incorporados(mejorintervalo) && nframes2>nframes_min
            ...
            % Primero recortamos las referencias que m�s frames tienen, para que la
            % comparaci�n sea con el mismo n�mero de frames
...
%             cambio=P1>P2; % Si P1>P2, indica que las nuevas referencias est�n intercambiadas respecto a las primeras
%             % Siempre hay que poner la peque en el numerador, para que no se quede
%             % sin precisi�n y redondee a 1
%             if ~cambio
%                 P_res1=P1/(P1+P2);
%             else
%                 P_res1=P2/(P1+P2);
...
% Recorto las referencias al tama�o deseado

nframes_final=min(cellfun(@(x) size(x,4),referencias)); % Por si no se han podido incluir todos los frames que se quer�an
for c_peces=1:n_peces
