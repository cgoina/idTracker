...
max_bwdist_act=min(max_bwdist_act,[],2); % As� coge el m�s peque�o de cada frame
...
%     if ~trozo2indiv(c_trozos)
        ...
        for sentido=[-1 1]
            ...
            % Comprueba que los que solapan no solapan con nadie m�s
            bien=true;
            ...
            % Si todo est� bien, intenta la resegmentaci�n
            if any(solapan_act) && bien && all(trozo2indiv(solapan_act))
                frames=find(any(trozos==c_trozos,2));
                frame_act=abs(min(sentido*frames));
                resegmentado=true;
                while ~isempty(frame_act) && frame_act>1 && frame_act<n_frames && resegmentado && any(trozos(frame_act,:)==c_trozos) % frame_act est� vac�o cuando se ha resegmentado el cruce entero en sentido contrario
                    fprintf('%g,',frame_act)
                    resegmentado=false;
